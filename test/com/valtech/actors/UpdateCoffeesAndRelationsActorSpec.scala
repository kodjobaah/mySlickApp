...
class UpdateCoffeesAndRelationsActorSpec(_system: ActorSystem) extends TestKit(_system) with WordSpecLike with Matchers with ImplicitSender with BeforeAndAfterAll {
  ...
  "when UpdateCoffee message is sent to UpdateCoffeesAndRelationsActor it" should {
    "send a UpdateResult message" in {
      ...
      running(FakeApplication(additionalConfiguration = inMemoryDatabase())) {
        val updateCoffeesAndSuppliers = system.actorOf(UpdateCoffeesAndRelationsActor.props)
        updateCoffeesAndSuppliers ! UpdateCoffee(Coffee("",0,0,0,0,0))
